...
namespace Calendar
{
    public class DayView : Control
    {
        ...
        public DayView()
        {
			...
			cachedAppointments.Add ( appointment.StartDate.Day, appointment );
        }
        ...
        private void EnterEditMode(object state)
        {
            ...
            if (this.InvokeRequired)
            {
				...
				object[] stateArray = new object[] {state};
                if (selectedApp == selectedAppointment)
                    this.Invoke(new StartEditModeDelegate(EnterEditMode), stateArray);
            }
            else
            {
                StartEditing();
		...
		public AppointmentViewDictionary appointmentViews = new AppointmentViewDictionary();
//      internal Dictionary<Appointment, AppointmentView> appointmentViews = new Dictionary<Appointment, AppointmentView>();
        ...
        private HalfHourLayout[] GetMaxParalelAppointments(AppointmentList appointments)
        {
