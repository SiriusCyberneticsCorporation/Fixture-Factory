...
namespace Calendar
{
    public class Office12Renderer : AbstractRenderer
    {
        ...
        public override void DrawAppointment(System.Drawing.Graphics g, System.Drawing.Rectangle rect, Appointment appointment, bool isSelected, int gripWidth)
        {
			...
			start = InterpolateColors(Color.Wheat, Color.White, 0.2f);
			end = InterpolateColors(Color.Wheat, Color.White, 0.6f);
			Rectangle titleBox = new Rectangle ( rect.X, rect.Y, rect.Width, 40 );
			using (LinearGradientBrush aGB = new LinearGradientBrush(titleBox, start, end, LinearGradientMode.Vertical))
				g.FillRectangle(aGB, titleBox);
